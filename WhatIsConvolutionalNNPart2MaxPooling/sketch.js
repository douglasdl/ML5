...
function setup() {
    ...
    createCanvas(dimension * 10 * 2, dimension * 10 * 2);
    filtered = createImage(dimension, dimension);
    pooled = createImage(dimension/stride, dimension/stride);
    background(255);
    noSmooth();
    image(cat, 0, 0, dimension * 10, dimension * 10);
    cat.loadPixels();
    
    // Convolution Layer
    filtered.loadPixels();
    // Loop through image pixels (ignoring the border pixels)
    ...
    image(filtered, dimension * 10, 0, dimension * 10, dimension * 10);


    // Pooling
    pooled.loadPixels();
    for(let x = 0; x < dimension - 1; x += stride) {
        for(let y = 0; y < dimension - 1; y += stride) {
    ...
    pooled.updatePixels();
    image(pooled, dimension * 10, dimension * 10, dimension * 10, dimension * 10);
}

function index(x, y, img) {
    return (x + y * img.width) * 4; // 4 channels RGBA
}
